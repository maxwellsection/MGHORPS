...
# 取消了全局 import，以打破 Python 启动时 2 秒的物理锁死现象！
# 全部引擎延迟按需加载
PDHG_AVAILABLE = True
...
# 延迟导入以加速启动
CUPY_AVAILABLE = True # 推迟到初始化再检测
PULP_AVAILABLE = True # 推迟到使用时再检测
...
class UltimateLPSolver:
    ...
    def __init__(self, tolerance=1e-8, solver='auto', use_gpu=False, use_npu=False, npu_cores=2, verbose_options=None):
        ...
        if use_gpu:
            try:
                import cupy as local_cp
                self.use_gpu = True
                self.array_lib = local_cp  # 使用CuPy进行GPU计算
            except ImportError:
                self.use_gpu = False
                self.array_lib = np
                print("⚠️ CuPy未安装，将降级使用CPU计算")
        else:
            self.use_gpu = False
    ...
    def _solve_problem(self, tableau_info, problem):
        ...
        if n_artificial > 0:
            if n_artificial <= 3:
                result = self._big_m_simplex(tableau, n_constraints, tableau_info, objective)
            else:
    ...
    def _big_m_simplex(self, tableau, n_constraints, tableau_info, objective):
        """大M法单纯形法"""
        self._log('basic', f"      🔄 使用大M法...")
        self.history.append({"method": "大M法", "action": "start"})
        
        M = 1e6
        is_max = objective['type'].lower() in ['max', 'maximize']
        m_penalty = -M if is_max else M
        ...
        for i in range(tableau_info['n_artificial']):
            tableau[-1, artificial_start + i] = m_penalty
            
        # 恢复由于初始基变量（这里人工变量作为基变量）导致的 reduced costs
        ...
        self._log('iterations', f"      📊 原目标函数加入罚金后恢复完成: {tableau[-1, :-1]}")
        
        # 直接使用单阶段算法（无需进入阶段1/2的分片记录）
        if is_max:
            tableau[-1, :] = -tableau[-1, :]
            
        self.history.append({"phase": "Big M", "action": "start"})
        phase_res = self._simplex_iterations(tableau, n_constraints, max_iter=1000)
        return {
            'status': phase_res['status'],
            'message': phase_res.get('message', ''),
            'tableau': phase_res.get('tableau'),
            'iterations': phase_res.get('iterations', 0),
    ...
    def _eliminate_artificial_variables(self, tableau, n_constraints, tableau_info):
        ...
        self.history.append({"phase": 1, "action": "start"})
    ...
    def _single_phase_simplex(self, tableau, n_constraints, objective):
        ...
        self._log('iterations', f"      🔄 开始单阶段/第二阶段单纯形法迭代...")
        self.history.append({"phase": 2, "action": "start"})
    ...
    def _simplex_iterations(self, tableau, n_constraints, max_iter=1000):
        ...
        while iteration < max_iter:
            self._log('tableau', f"\n--- 迭代 {iteration} 单纯形表 ---\n{tableau}\n")
            current_tableau = tableau.copy() if hasattr(tableau, 'copy') else tableau
            step_record = {
                "iteration": iteration,
            ...
            if not self.array_lib.any(positive_mask):
                ...
                step_record["status"] = "unbounded"
                step_record["message"] = "问题无界"
                return {
                    'status': 'unbounded',
                    'message': '问题无界',
                    'history': self.history
            ...
            if abs(pivot_element) < self.tolerance:
                ...
                step_record["status"] = "numerical_error"
                return {
            ...
            step_record["pivot_row"] = int(pivot_row)
            step_record["pivot_col"] = int(pivot_col)
            step_record["message"] = f"换基: 进基变量为 x{int(pivot_col)+1}，出基变量在行 {int(pivot_row)}"
    ...
    def _process_result(self, result, problem, variables):
        ...
        if self.use_gpu:
            reduced_costs = self.array_lib.asnumpy(reduced_costs)
            dual_prices = self.array_lib.asnumpy(dual_prices)
    ...
    def _extract_solution(self, tableau, tableau_info, problem):
        ...
        if self.use_gpu:
            solution = self.array_lib.asnumpy(solution)
