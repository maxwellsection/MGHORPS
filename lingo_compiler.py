...
class LingoCompiler:
    ...
    def _resolve_sums(self, text: str) -> str:
        """处理 @SUM 宏展开，支持嵌套括号"""
        while True:
            blocks = self._extract_blocks(text, "@SUM")
            if not blocks:
                break
            ...
            iterator_decl = parts[0].strip()
            inner_expr = parts[1].strip()
            
            # iterator_decl 形如 CITIES(i)
            ...
            if set_name not in self.sets:
                raise ValueError(f"Unknown set in @SUM: {set_name}")
        ...
        if stmt.upper().startswith('MAX') or stmt.upper().startswith('MIN'):
            if '=' in stmt:
                ...
                if self._is_nlp(expr):
                    ...
                    # We still need to extract variables from the raw expression!
                    # A naive approach: find all words
